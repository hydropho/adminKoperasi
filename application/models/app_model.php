...
class App_Model extends CI_Model
{
	...
	{
		$t = "SELECT * FROM gudang WHERE id_gudang='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if ($r > 0) {
			foreach ($d->result() as $h) {
	...
	public function ItemPesan($id)
	{
		$t = "SELECT kode_pesan FROM d_pesan WHERE kode_pesan='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if ($r > 0) {
			$hasil = $r;
	...
	public function CariJmlBeli($kode, $tgl_awal, $tgl_akhir)
	{
		$t = "SELECT kode_barang,sum(jmlbeli) as jml
				FROM d_beli as a
				JOIN h_beli as b
				ON a.kodebeli = b.kodebeli
				WHERE kode_barang='$kode' 
				AND (tglbeli BETWEEN '" . $tgl_awal . "' AND '" . $tgl_akhir . "' OR tglbeli<='" . $tgl_awal . "')
	...
	public function CariJmlJual($kode, $tgl_awal, $tgl_akhir)
	{
		$t = "SELECT kode_barang,sum(jmljual) as jml 
				FROM d_jual as a 
				JOIN h_jual as b
				ON a.kodejual = b.kodejual
				WHERE kode_barang='$kode' 
				AND (tgljual BETWEEN '" . $tgl_awal . "' AND '" . $tgl_akhir . "' OR tgljual<='" . $tgl_awal . "')
				GROUP BY kode_barang";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if ($r > 0) {
	...
	public function CariStokAkhir($kode)
	{
		...
		$beli = $this->app_model->CariJmlBeli($kode, $tgl, $tgl);
		$jual = $this->app_model->CariJmlJual($kode, $tgl, $tgl);
	...
	public function MaxKodePesan()
	{
		$bln = date('m');
		$th = date('y');
		$text = "SELECT max(kode_pesan) as no FROM h_pesan";
		$data = $this->app_model->manualQuery($text);
		if ($data->num_rows() > 0) {
			foreach ($data->result() as $t) {
				$no = $t->no;
				$tmp = ((int) substr($no, 5, 5)) + 1;
				$hasil = 'PS' . sprintf("%05s", $tmp);
			}
		} else {
			$hasil = 'PS' . '00001';
		}
	...
	public function MaxKodeBarang()
	{
		$bln = date('m');
		$th = date('y');
		$text = "SELECT max(kode_barang) as no FROM barang";
		$data = $this->app_model->manualQuery($text);
		if ($data->num_rows() > 0) {
			foreach ($data->result() as $t) {
				$no = $t->no;
				$tmp = ((int) substr($no, 5, 5)) + 1;
				$hasil = 'BR' . sprintf("%05s", $tmp);
			}
		} else {
			$hasil = 'BR' . '00001';
		}
	...
	//Latihan Membuat Model jurusan

	public function Nama_Jurusan($id)
	{
		$t = "SELECT * FROM jurusan WHERE kode_jurusan='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if ($r > 0) {
			foreach ($d->result() as $h) {
				$hasil = $h->nama_jurusan;
			}
		} else {
			$hasil = '';
		}
